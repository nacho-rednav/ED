...
int operacion(int iz, int dr, string op) {
    if (op == "+") return iz + dr;
    else if (op == "-") return iz - dr;
    else if (op == "*") return iz * dr;
}

/*Costes:
En funcion de S (número de nodos): La función operar es de coste O(S) 
Esta función no depende de M (número de asignaciones del programa)
En función de N (número de variables):
Como mínimo habrá 2N + 1 nodos, como máximo es impredecible ya que puede extenderse
la expresión sin añadir variables nuevas.
Por lo que suponemos que el coste será O(S)
*/
int operar(BinTree<string> t, MapTree<string, int> dict) {
    char c = t.root()[0];
    if (t.left().empty() && t.right().empty()) {
        ...
        int iz = operar(t.left(), dict);
        int dr = operar(t.right(), dict);
        return operacion(iz, dr, t.root());
    }

}

/*Costes:
En función de M, número de asignaciones el coste es O(M)
En función de S, número de nodos del árbol, el coste sería O(M * N)
también dependería de M, número de asignaciones, debido al bucle
En función de N, número de variables, el coste sería O(M * S)
*/
bool tratar_caso() {
    int n;
    BinTree<string> t;
    MapTree<string, int> dict;
    cin >> n;
    if (n == 0) return false;
    for (int i = 0; i < n; i++) {
        t = parse(cin);
        dict[t.left().root()] = operar(t.right(), dict);
    }
