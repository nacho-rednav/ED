...
//Primera componente: maxima diferencia encontrada en el hijo
//Segunda componente: ultimo nodo visitado en el hijo
//La funcion visita cada nodo una única vez, por lo que su coste es lineal respecto al número de nodos
pair<int, int> maximo_ultimo(const BinTree<int>& t) {
    pair<int, int> res(0, t.root()), aux(0, 0);

    if (!t.left().empty()) {
        res.first = abs(t.root() - t.left().root());
        aux = maximo_ultimo(t.left());
        res.first = max(res.first, aux.first);
        res.second = aux.second;
    }
    if (!t.right().empty()) {
        res.first = max(res.first, abs(res.second - t.right().root()));
        aux = maximo_ultimo(t.right());
        res.first = max(res.first, aux.first);
        res.second = aux.second;
    }
    return res;
...
//El coste de esta función es el coste de la función maximo_ultimo(), O(n), ya que recorre todos los nodos del árbol solamente una única vez cada uno
...
// Coste O(n), siendo n el número de nodos del árbol
//El coste de estaa función es el coste de la función preorder(), O(n), ya que recorre todos los nodos del árbol solamnete una única vez cada uno
int maxima_diferencia_Preorder(const BinTree<int>& t) {
    // Implementa aquí la función. No puedes modificar el tipo
    // de entrada ni de salida. No obstante, puedes apoyarte en
    // funciones auxiliares recursivas, siempre que tengan
    // un único parámetro de entrada (de tipo const BinTree<int> &)
    ...
    if (!t.empty()) {
        pre = t.root();
        t.preorder([&maxi, &pre](int x) {
            maxi = max(maxi, abs(pre - x));
            pre = x;
            });
    }
    return maxi;
