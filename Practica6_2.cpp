...
/*
    T(n) = { k1 n = 0
           { T(ni) + T(nd) + k2 n > 0
    Esta recurrencia, como hemos visto en la teoría, tiene coste
    O(n) siendo n el número de nodos del árbol de entrada
...
Data altura_equilibradoEstable_hojas(const BinTree<T>& arbol) {
    ...
    if (arbol.empty()) {
        ...
        res.altura = 1 + max(izq.altura, der.altura);//Lo demás O(1)
...
//tiempo será el de esa función que es O(n), siendo n el número de nodos
