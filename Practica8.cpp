...
//Suponemos que las cartas vendrán de tal forma que los árboles del TAD Conjunto sean equilibrados
// El coste será O(cartas * log(n)), siendo n el número de nodos del árbol del jugador que corresponde = num cartas / num jugadores
void resolver(int n, vector<int> const& nums) {
    vector<SetTree<int>> jugadores(n);

    for (int i = 0; i < nums.size(); i++) { //O(cartas)
        if (!jugadores[i % n].contains(nums[i])) { //O(log nodos) suponiendo equilibrado
            jugadores[i % n].insert(nums[i]); //O(log nodos)
        }
        else {
            jugadores[i % n].erase(nums[i]);//O(log nodos)
        }
    }
    for (int i = 0; i < n; i++) {//O(nJugadores), su coste asintótico será menor que el de el  bucle de arriba
        cout << "J" << i + 1 << ": " << jugadores[i] << "\n";
    }
    cout << "---\n";
}
